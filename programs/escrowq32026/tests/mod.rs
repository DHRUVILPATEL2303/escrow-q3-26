...
fn test_make_and_refund() {
    ...
    println!("'Refund' successful. Escrow and Vault closed.");
}

#[test]
fn test_make_update_and_take() {
    println!("\n Starting test_make_update_and_take");
    let (mut program, payer) = setup();
    let maker = payer.pubkey();
    ...
    MintTo::new(&mut program, &payer, &mint_a, &maker_ata_a, 10_000_000)
        .send()
        .unwrap();
    MintTo::new(&mut program, &payer, &mint_b, &taker_ata_b, 20_000_000)
        .send()
        .unwrap();
    ...
    assert_eq!(maker_b_data.amount, 20_000_000);
